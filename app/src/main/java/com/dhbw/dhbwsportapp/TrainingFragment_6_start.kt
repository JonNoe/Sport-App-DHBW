...
class TrainingFragment_6_start : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_training_6_start, container, false)
    ...
    private fun navigateToPauseFragment() {
        if (currentExerciseIndex < exerciseTitles.size - 1) {
            val trainingBreakFragment = TrainingFragment_6_break()
            val args = Bundle()
    ...
    private fun navigateToResultFragment() {
        if (isAdded) {
            val resultFragment = TrainingFragment_6_result()
            val transaction = requireActivity().supportFragmentManager.beginTransaction()
